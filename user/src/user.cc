...
static unsigned long *UTCB_TRAMP_RECV = reinterpret_cast<unsigned long *>(UTCBS_START + 8192);

const unsigned long  ROOT_PD_SEL      = 0;
const unsigned long  INV_CAP          = 512;
...
unsigned long sys_create_pd(unsigned long del_sel)
{
    unsigned long sel = next_sel++;
    syscall3(6, sel, del_sel);
    return sel;
}
...
void tramp_portal()
{
    unsigned long *words = UTCB_TRAMP_RECV;
    words[0]             = INV_CAP; // rcv_typed
    words[1]             = pt_sel;  // snd_typed (portal)
    words[2]             = 0;       // untyped
}
...
void sender()
{
    ...
    unsigned long  pt_sel = next_sel++;

    words[0]              = pt_sel;  // rcv_typed (sel for portal)
    words[1]              = INV_CAP; // snd_typed (none)
    words[2]              = 0;       // untyped
    sys_call(tramppt_sel);

    // call obtained portal
    words[0] = INV_CAP;
    words[1] = INV_CAP;
    words[2] = 2;
    words[3] = 1;
...
void main_func()
{
    unsigned long stack[STACK_WORDS * 3];

    // create trampoline PT and EC
    unsigned long trampec_sel =
        sys_create_ec(ROOT_PD_SEL, 0, local_ec_stack(stack + STACK_WORDS * 3), UTCB_TRAMP_RECV);
    tramppt_sel = sys_create_pt(tramp_portal, trampec_sel);

    // sender thread in new PD (and pass trampoline portal)
    unsigned long sndpd_sel = sys_create_pd(tramppt_sel);
    sys_create_ec(sndpd_sel, sender, stack + STACK_WORDS * 1, UTCB_SENDER);

    // receiver thread in root PD
    unsigned long rcvec_sel =
