...
class Atomic
{
    public:
        ...
        template <typename T>
        [[gnu::always_inline]]
        static inline void set_mask(T &ptr, T v)
        {
            __sync_or_and_fetch(&ptr, v);
        }

        template <typename T>
        [[gnu::always_inline]]
        static inline void clr_mask(T &ptr, T v)
        {
            __sync_and_and_fetch(&ptr, ~v);
        }
